...
- github.branch_pr_flow :: Flusso di default: un task = un branch = una PR. Merge non squash-only: squash, merge commit e rebase. Niente `required_linear_history`. Push diretto su `main` (bypass owner Always) per il flusso quotidiano locale senza aspettare Actions; non usarlo per il merge di PR rosse. Convenzioni in `docs/development/WORKFLOWS.md` § Branching, PR e worktree.
- github.main_ruleset :: Ruleset `Default` su `~DEFAULT_BRANCH`: deletion, non_fast_forward, PR (1 approve, CODEOWNERS, dismiss stale, extra approval unattributed; no last-push-approval, no thread resolution), required checks `quality` `integration-and-coverage` `bun-compatibility` `e2e` `dependency-review`, bypass user Always. `delete_branch_on_merge` acceso. CodeQL resta slow lane, non è required check.
